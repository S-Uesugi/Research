...
string MakeResultDirectory(string root_result_path,vector<string> data_list,string nroot,int M ,int rotation){
  bool flag = true;
  int path_length = data_list[0].length();
  int split_num;
  string result_path;
  while(flag){
    string main_path = root_result_path+nroot+"_error"+"/";
    cout << main_path << endl;
...
/* argv[1] : the primitive root of the unity, argv[2] : embedding factor, arg[3] : rotation, arg[4] : blocksize */
int main(int argc, char * argv[]){  
  ...
  for(auto& data : filesystem::directory_iterator(path)){
    if(string(data.path()).find(argv[1]) != string::npos) data_list.push_back(string(data.path())); 
  }
  if(data_list.empty()){
    cout << "Please input the correct number again."<<endl;
    exit(0);
  }
  sort(data_list.begin(),data_list.end());

  int exp_root = atoi(argv[1]);
  int M  =  atoi(argv[2]);
  ...
  string filename(data_list[0] + "/parameters.csv");
  string filename1(data_list[0] + "/Sample_A1.csv");
  string filename2(data_list[0] + "/Sample_b1.csv");
  string filename3(data_list[0] + "/Sample_A2.csv");
  string filename4(data_list[0] + "/Sample_b2.csv");
  string filename5(data_list[0] + "/error1.csv");
  string filename6(data_list[0] + "/error2.csv");
  string root_result_path = "../result/search/rlwe";
  string result_path = MakeResultDirectory(root_result_path,data_list,argv[1],M,rotation);
  ...
  for(int i = 0;i < exp_num;i++){
    ...
    for(int j = 0;j < rotation+1;j++){
        if(minus_check(B[j],rotation,dim)) Reversal(B[j]);
        vec_ZZ b = extract_target(B[j],rotation);
        cand.push_back(b);
    }
